...
smke = Animation("smoke.png",4,game,1024/4,256,frate=6)
smke.resizeBy(-35)

human = Animation("Human.png",4,game,1600/4,400,frate=12)
...
while not game.over:
    ...
    lava.draw()
    smke.draw()
    smke.moveTowards(human,3)
    'smke.rotateTowards(human)'
    human.draw()
    human.moveTowards(hero,3)
    platform.draw()
    platform2.draw()
    platform3.draw()
    platform4.draw()
    ...
    if not hero.collidedWith(platform) or hero.collidedWith(platform2) or hero.collidedWith(platform3) or hero.collidedWith(platform4) or hero.collidedWith(platform5):
        landed = False
    else:
        landed = True
